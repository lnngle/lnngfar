...
<!-- 示例：每个功能从独立库开始；库必须自包含、可独立测试、文档齐全；目标清晰——禁止仅为组织而拆库 -->
...
<!-- 示例：必须 TDD：先写测试 → 获得确认 → 测试失败 → 再实现；严格遵循红-绿-重构循环 -->

### [PRINCIPLE_4_NAME]
<!-- 示例：IV. 集成测试 -->
[PRINCIPLE_4_DESCRIPTION]
<!-- 示例：需覆盖集成测试的重点：新库契约测试、契约变更、服务间通信、共享模式 -->

### [PRINCIPLE_5_NAME]
<!-- 示例：V. 可观测性、VI. 版本与破坏性变更、VII. 简洁性 -->
[PRINCIPLE_5_DESCRIPTION]
<!-- 示例：文本 I/O 保障可调试性；要求结构化日志；或采用 MAJOR.MINOR.BUILD 版本格式；或遵循先简后繁与 YAGNI 原则 -->
...
## 治理
<!-- 示例：宪章高于其他实践；修订需文档化、审批并附迁移计划 -->

[GOVERNANCE_RULES]
<!-- 示例：所有 PR/评审必须核验合规；复杂度必须有正当理由；运行期开发请遵循 [GUIDANCE_FILE] -->
